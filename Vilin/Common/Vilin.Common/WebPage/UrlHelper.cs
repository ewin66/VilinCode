...
namespace ComLib
{
    ...
    public class UrlHelper
    {
        ...
        #region "方法用于程序可能有web调用和其它调用 返回文件的绝对路径"
        /// <summary>
        /// 方法用于程序可能有web调用和其它调用 返回文件的绝对路径
        /// </summary>
        /// <param name="fileName"></param>
        /// <returns></returns>
        public static string AppConfig(string fileName)
        {
            String realname = "";
            HttpContext context = HttpContext.Current;
            if (context != null)
                realname = context.Server.MapPath(fileName);
            else
                realname = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, fileName.Remove(0, 2));
