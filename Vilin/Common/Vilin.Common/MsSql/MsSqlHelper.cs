...
namespace JF.Common.Libary.DataAccess
{
    ...
    public sealed class MsSqlHelper
    {
        ...
        public static readonly string ConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings[""].ToString();

        private static int CmdTimeout = (ConfigurationManager.AppSettings["CommandTimeout"] == null
                                             ? 900
                                             : Convert.ToInt32(ConfigurationManager.AppSettings["CommandTimeout"]));
        ...
        /// <param name="Size">parameter length .</param>
        /// <param name="Value">parameter value .</param>
        /// <param name="Direction">input output ·½Ïò.</param>
        /// <returns>parameter object.</returns>
                                             ...
                                             ParameterDirection Direction, object Value)
        {
            ...
            {
                DateTime tempDate = Convert.ToDateTime(Value);
                if (tempDate == (DateTime)SqlDateTime.Null)
                {
                    Value = SqlDateTime.Null.ToSqlString();
                }
            }
                                             ...
                                             params SqlParameter[] commandParameters)
        {
            //create a command and prepare it for execution
            SqlCommand cmd = new SqlCommand();
            //==modify by John 2011-08-25 start
            cmd.Transaction = connection.BeginTransaction(IsolationLevel.ReadUncommitted);
            //PrepareCommand(cmd, connection, (SqlTransaction) null, commandType, commandText, commandParameters);
            PrepareCommand(cmd, connection, cmd.Transaction, commandType, commandText, commandParameters);
        ...
        #region Added by Liny,2011-12-22,for getting the output parameters of SP
        ...
        public static DataTable ExecuteDataTable(SqlConnection connection, CommandType commandType, string commandText, SqlParameter[] commandParameters, out SqlParameter[] outputParameters)
        {
            ...
            if (null != commandParameters && commandParameters.Length > 0)
            {
                int count = 0;
                foreach (SqlParameter p in commandParameters)
                {
                ...
                outputParameters = new SqlParameter[count];
                count = -1;
                foreach (SqlParameter p in commandParameters)
                {
                    if (p.Direction == ParameterDirection.Output || p.Direction == ParameterDirection.InputOutput)
                    {
                        outputParameters[++count] = (SqlParameter)cmd.Parameters[p.ParameterName];
                    }
                }
            }
        ...
        #endregion Added by Liny,2011-12-22,for getting the output parameters of SP
        ...
        /// <remarks>
        /// e.g.:
        ///  DataTable dt = ExecuteDataTable(Framework.ConnString, CommandType.StoredProcedure, "GetOrders");
        /// </remarks>
        /// <param name="connectionString">a valid connection string for a SqlConnection</param>
        /// <param name="commandType">the CommandType (stored procedure, text, etc.)</param>
        /// <param name="commandText">the stored procedure name or T-SQL command</param>
        ...
        /// <remarks>
        /// e.g.:
        ///  DataTable dt = ExecuteDataTable(Framework.ConnString, CommandType.StoredProcedure, "GetOrders", new SqlParameter("@prodid", 24));
        /// </remarks>
        /// <param name="connectionString">a valid connection string for a SqlConnection</param>
        /// <param name="commandType">the CommandType (stored procedure, text, etc.)</param>
        /// <param name="commandText">the stored procedure name or T-SQL command</param>
                                                  ...
                                                  params SqlParameter[] commandParameters)
        {
            ...
            try
            {
                ...
                return
                    ExecuteReader(cn, null, commandType, commandText, commandParameters, SqlConnectionOwnership.Internal);
            }
            catch
            {
                                                  ...
                                                  params SqlParameter[] commandParameters)
        {
            ...
            return
                ExecuteReader(connection, (SqlTransaction)null, commandType, commandText, commandParameters,
                              SqlConnectionOwnership.External);
        }

        //==add by John 2011-08-25 start
        /// <summary>
        /// Execute a SqlCommand (that returns a resultset) against the specified SqlConnection
        /// using the provided parameters.
        /// </summary>
        ...
        /// <param name="commandParameters">an array of SqlParamters used to execute the command</param>
        /// <returns>a SqlDataReader containing the resultset generated by the command</returns>
        public static SqlDataReader ExecuteReader(SqlConnection connection, CommandType commandType, SqlTransaction transaction, string commandText,
                                                  params SqlParameter[] commandParameters)
        {
            //pass through the call to the private overload using a null transaction value and an externally owned connection
            return
                ExecuteReader(connection, transaction, commandType, commandText, commandParameters,
                              SqlConnectionOwnership.External);
        }

        //==add by John 2011-08-25 end
                                           ...
                                           params SqlParameter[] commandParameters)
        {
            ...
            //if (cmd.Transaction.IsolationLevel == IsolationLevel.ReadUncommitted)
            //    cmd.Transaction.Commit();
